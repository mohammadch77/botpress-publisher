...
beforeEach(function () {
    Http::fake([
        'api.telegram.org/*' => Http::response(['ok' => true, 'result' => ['message_id' => 1, 'chat' => ['id' => 111]]]),
    ]);
...
it('returns 200 for an unknown bot uuid', function () {
    $response = $this->postJson('/webhook/telegram/'.Str::uuid(), [
        'update_id' => 1,
...
it('returns 200 for a valid bot', function () {
    $response = $this->postJson("/webhook/telegram/{$this->bot->uuid}", [
        'update_id' => 1,
        'message' => ['message_id' => 1, 'date' => time(), 'chat' => ['id' => 111, 'type' => 'private'], 'from' => ['id' => 222], 'text' => 'hi'],
    ]);
...
it('parses the payload and creates a session', function () {
    $this->postJson("/webhook/telegram/{$this->bot->uuid}", [
        'update_id' => 1,
        'message' => ['message_id' => 1, 'date' => time(), 'chat' => ['id' => 111, 'type' => 'private'], 'from' => ['id' => 222], 'text' => 'hi'],
    ]);
...
it('creates a platform identity for a new user', function () {
    ...
    $identity = PlatformIdentity::withoutGlobalScopes()->where('platform', 'telegram')->where('external_user_id', '222')->first();
